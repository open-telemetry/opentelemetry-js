...
Configure resource detectors. By default, the resource detectors are
[processDetector, hostDetector, envDetector]. When `OTEL_SERVICE_NAME` is set,
the serviceDetector also runs before envDetector.
...
The `envDetector` always runs last so `OTEL_RESOURCE_ATTRIBUTES` is honored
and can override attributes detected by other resource detectors.

**NOTE:** `os` is a Node.js-specific detector with no equivalent in the [OpenTelemetry declarative configuration spec](https://github.com/open-telemetry/opentelemetry-configuration).

For example, to enable only the `host` detector in addition to the always-enabled `envDetector`:

```shell
export OTEL_NODE_RESOURCE_DETECTORS="host"
```

The order set on `OTEL_NODE_RESOURCE_DETECTORS` is respected for configurable
detectors. The `envDetector` always runs after them, so a
`service.instance.id` from `OTEL_RESOURCE_ATTRIBUTES` takes precedence over the
random UUID generated by the `serviceDetector`.

...
describe('MetricDataSplitter', () => {
  ...
  it('should split batches when exceeding maxExportBatchSize (SUM)', () => {
    ...
    assert.strictEqual(batches.length, 2);
    assert.strictEqual(batches[0].scopeMetrics[0].metrics[0].dataPoints.length, 2);
    assert.strictEqual((batches[0].scopeMetrics[0].metrics[0] as any).isMonotonic, true);
    assert.strictEqual(batches[1].scopeMetrics[0].metrics[0].dataPoints.length, 1);
  });
  ...
  it('should split batches when exceeding maxExportBatchSize (HISTOGRAM)', () => {
    const dummyHistogram = { buckets: { boundaries: [1, 2], counts: [1, 1, 1] }, sum: 3, count: 3 };
    const resourceMetrics: ResourceMetrics = {
      resource: dummyResource,
      scopeMetrics: [
        {
          ...
          metrics: [
            {
              ...
              dataPoints: [
                { startTime: [0, 0], endTime: [0, 0], attributes: {}, value: dummyHistogram },
                { startTime: [0, 0], endTime: [0, 0], attributes: {}, value: dummyHistogram },
                { startTime: [0, 0], endTime: [0, 0], attributes: {}, value: dummyHistogram },
              ],
              descriptor: { name: 'm1', description: 'desc1', unit: 'unit1', valueType: ValueType.INT },
  ...
  it('should split batches when exceeding maxExportBatchSize (EXPONENTIAL_HISTOGRAM)', () => {
    const dummyExponentialHistogram = { sum: 3, count: 3, scale: 1, zeroCount: 0, positive: { offset: 1, bucketCounts: [1, 1, 1] }, negative: { offset: 1, bucketCounts: [] } };
    const resourceMetrics: ResourceMetrics = {
      resource: dummyResource,
      scopeMetrics: [
        {
          ...
          metrics: [
            {
              ...
              dataPoints: [
                { startTime: [0, 0], endTime: [0, 0], attributes: {}, value: dummyExponentialHistogram },
                { startTime: [0, 0], endTime: [0, 0], attributes: {}, value: dummyExponentialHistogram },
                { startTime: [0, 0], endTime: [0, 0], attributes: {}, value: dummyExponentialHistogram },
              ],
              descriptor: { name: 'm1', description: 'desc1', unit: 'unit1', valueType: ValueType.INT },

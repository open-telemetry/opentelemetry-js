...
import { OnDemandExportingMetricReader } from '../../src/export/OnDemandExportingMetricReader';
...
describe('OnDemandExportingMetricReader', () => {
  afterEach(() => {
    sinon.restore();
  });

  describe('constructor', () => {
    it('should construct OnDemandExportingMetricReader without exceptions', () => {
      const exporter = new TestDeltaMetricExporter();
      assert.doesNotThrow(
        () =>
          new OnDemandExportingMetricReader({
            exporter,
  ...
  describe('setMetricProducer', () => {
    it('should not start exporting periodically', async () => {
      const exporter = new TestDeltaMetricExporter();
      const exporterMock = sinon.mock(exporter);
      exporterMock.expects('export').never();

      const reader = new OnDemandExportingMetricReader({
        exporter: exporter,

...
export class ZipkinExporter implements SpanExporter {
  ...
  constructor(config: zipkinTypes.ExporterConfig = {}) {
    ...
    this._send = prepareSend(this._diagLogger, urlStr, config.headers);
  ...
  export(
    ...
    this._diagLogger.debug('Zipkin exporter export');
    if (this._isShutdown) {
      setTimeout(() =>
        resultCallback({
  ...
  shutdown(): Promise<void> {
    this._diagLogger.debug('Zipkin exporter shutdown');

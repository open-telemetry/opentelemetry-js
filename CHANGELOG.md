...
* feat(sdk-trace-web): implement `attach`/`detach` on `StackContextManager` (full synchronous support). [#6387](https://github.com/open-telemetry/opentelemetry-js/pull/6387) @pichlermarc
* feat(context-zone-peer-dep): implement best-effort `attach`/`detach` on `ZoneContextManager`; composes with `with()` but is not isolated across concurrent async zones. [#6387](https://github.com/open-telemetry/opentelemetry-js/pull/6387) @pichlermarc
* feat(context-async-hooks): implement `attach`/`detach` on `AsyncLocalStorageContextManager` (full async support, propagates across asynchronous boundaries). [#6387](https://github.com/open-telemetry/opentelemetry-js/pull/6387) @pichlermarc
  * The deprecated `AsyncHooksContextManager` implements `attach`/`detach` as a no-op; use `AsyncLocalStorageContextManager` for full support.

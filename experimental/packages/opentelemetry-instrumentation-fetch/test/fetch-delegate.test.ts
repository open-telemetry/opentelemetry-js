...
describe('fetch', () => {
  ...
  describe('enabling/disabling', () => {
    ...
    it('should wrap global fetch when instantiated', () => {
      ...
      fetchInstrumentation = createFetchInstrumentation();
      assert.ok(isWrapped(window.fetch));
    });

    it('should not wrap global fetch when instantiated with `enabled: false`', () => {
      assert.ok(!isWrapped(window.fetch));
      fetchInstrumentation = createFetchInstrumentation({ enabled: false });
      assert.ok(!isWrapped(window.fetch));
  ...
  describe('instrumentation', () => {
    ...
    const trace = async (
      ...
      try {
        ...
        const fetchInstrumentation = createFetchInstrumentation(config);

 ...
 * Packages should extend this config in their vitest.browser.config.mts:
 *
...
export default defineConfig({
  resolve: {
    alias: {
      ...
      'node:module': new URL(
        './scripts/test-stubs/node-module.mjs',
        import.meta.url
      ).pathname,
      // Use Chai's assert (bundled with Vitest) instead of Node.js polyfill
      assert: new URL('./scripts/test-stubs/assert.mjs', import.meta.url)
        .pathname,
    },
  },
  optimizeDeps: {

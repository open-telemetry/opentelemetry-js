...
class AwsEc2Detector implements Detector {
  ...
  private async _fetchToken(): Promise<string> {
    ...
    // const url = this.AWS_INSTANCE_TOKEN_DOCUMENT_URI;
  ...
  private async _fetchIdentity(token: string): Promise<any> {
    ...
    // const url = this.AWS_INSTANCE_IDENTITY_DOCUMENT_URI;
  ...
  private async _fetchHost(token: string): Promise<string> {
    ...
    // const url = this.AWS_INSTANCE_HOST_DOCUMENT_URI;
    return await this._fetchString(options);
  }
...
export const awsEc2DetectorTemp = new AwsEc2Detector();

...
/**
 * A detector that reads attributes from the OTEL_RESOURCE_ATTRIBUTES
 * environment variable.
 */
export class ResourceAttributesEnvDetector implements ResourceDetector {
  ...
  detect(_config?: ResourceDetectionConfig): DetectedResource {
    ...
    if (rawAttributes) {
      try {
        ...
        diag.debug(
          `ResourceAttributesEnvDetector failed: ${e instanceof Error ? e.message : e}`
        );
      }
    }
   ...
   */
  protected _parseResourceAttributes(rawEnvAttributes?: string): Attributes {
...
export const resourceAttributesEnvDetector =
  new ResourceAttributesEnvDetector();

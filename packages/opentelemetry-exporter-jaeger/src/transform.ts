...
import {CanonicalCode, Link} from '@opentelemetry/types';

import {Log, Tag, TagValue, ThriftLog, ThriftReference, ThriftReferenceType, ThriftSpan, ThriftTag, ThriftUtils, Utils,} from './types';
...
export function spanToThrift(span: ReadableSpan): ThriftSpan {
  ...
  const tags =
      Object.keys(span.attributes)
          .map(
              (name): Tag =>
                  ({key: name, value: toTagValue(span.attributes[name])}));
  ...
  const logs = span.events.map((event): Log => {
    ...
    if (attrs) {
      Object.keys(attrs).forEach(
          attr => fields.push({key: attr, value: toTagValue(attrs[attr])}));
    }
  ...
  return {
    ...
    duration: Utils.encodeInt64(Math.round(
...
function spanLinksToThriftRefs(
    links: Link[], parentSpanId?: string): ThriftReference[] {
  return links
             .map(
                 (link):
                     ThriftReference|
                 null => {
                   if (link.spanContext.spanId === parentSpanId) {
                     ...
                     const traceIdHigh =
                         Utils.encodeInt64(traceId.slice(0, 16));

...
export interface ExemplarReservoir {
   ...
   */
  collect(pointAttributes: SpanAttributes): Exemplar[];
}
...
class ExemplarBucket {
  ...
  offer(value: number, timestamp: HrTime, attributes: SpanAttributes, ctx: Context) {
    this.value = value;
  ...
  collect(pointAttributes: SpanAttributes): Exemplar | null {
...
export abstract class FixedSizeExemplarReservoirBase implements ExemplarReservoir {
  ...
  abstract offer(value: number, timestamp: HrTime, attributes: SpanAttributes, ctx: Context): void;

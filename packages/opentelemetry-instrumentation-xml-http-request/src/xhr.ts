...
export class XMLHttpRequestInstrumentation extends InstrumentationBase<XMLHttpRequest> {
   ...
   */
  enable() {
    this._diagLogger.debug('applying patch to', this.moduleName, this.version);

    if (isWrapped(XMLHttpRequest.prototype.open)) {
      this._unwrap(XMLHttpRequest.prototype, 'open');
      this._diagLogger.debug('removing previous patch from method open');
    }

    if (isWrapped(XMLHttpRequest.prototype.send)) {
      this._unwrap(XMLHttpRequest.prototype, 'send');
      this._diagLogger.debug('removing previous patch from method send');
    }

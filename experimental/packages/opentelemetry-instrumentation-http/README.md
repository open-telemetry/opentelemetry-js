...
`ignoreIncomingPropagationHook`         | `IgnoreIncomingPropagationFunction`        | Function for disabling propagation context extraction on incoming requests. HTTP instrumentation will not extract the propagation context from incoming requests for which the function returns `true`; the request is still traced but the server span will not have a remote parent. Useful, e.g., to ignore trace context from untrusted sources based on the request's IP address.
...
`IgnoreIncomingPropagationFunction`        | `request: IncomingMessage`                                                                                   | `true` skips extracting the propagation context from the incoming request; `false` extracts it

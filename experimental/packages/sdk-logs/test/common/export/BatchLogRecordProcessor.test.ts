...
import { ExportResultCode, getEnv, loggingErrorHandler, setGlobalErrorHandler } from "@opentelemetry/core";
...
const createLogRecord = (limits?: LogRecordLimits, shutdown?: boolean): LogRecord => {
...
describe("BatchLogRecordProcessorBase", () => {
  ...
  describe("onEmit", () => {
    it("should export the log records with buffer size reached", (done) => {
      ...
      const processor = new BatchLogRecordProcessor(exporter, defaultBufferConfig);
      for (let i = 0; i < defaultBufferConfig.maxExportBatchSize; i++) {
        ...
        // @ts-expect-error
        processor.onEmit(logRecord._toReadable());
        assert.strictEqual(exporter.getFinishedLogRecords().length, 0);
      }
      const logRecord = createLogRecord();
      // @ts-expect-error
      processor.onEmit(logRecord._toReadable());
      setTimeout(async () => {
        assert.strictEqual(exporter.getFinishedLogRecords().length, defaultBufferConfig.maxExportBatchSize);
    ...
    it("should export each log record exactly once with buffer size  reached multiple times", (done) => {
      ...
      originalTimeout(() => {
        ...
        originalTimeout(async () => {
          clock.tick(defaultBufferConfig.scheduledDelayMillis + 10);
          clock.restore();
          assert.strictEqual(exporter.getFinishedLogRecords().length, totalLogRecords + 1);
          await processor.shutdown();
          assert.strictEqual(exporter.getFinishedLogRecords().length, 0);
          done();
        });

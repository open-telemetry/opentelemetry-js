...
* feat(instrumentation-http): add an experimental `useDiagnosticsChannel` option (or `OTEL_INSTRUMENTATION_HTTP_USE_DIAGNOSTICS_CHANNEL` environment variable) to instrument `http`/`https` through the `node:http` diagnostics channels, falling back to module patching on runtimes without the channels (Node.js <22.12.0, or <23.2.0 on the 23.x line)

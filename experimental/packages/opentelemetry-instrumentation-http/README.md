...
- Final request authority changes are reflected in exported spans and metrics, but the resulting `server.address`, `server.port`, and `url.full` values are unavailable to OpenTelemetry SDK Samplers when the span starts.

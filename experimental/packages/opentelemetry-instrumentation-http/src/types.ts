 ...
 * creating a client span for that request. In diagnostics-channel mode these
 * options are reconstructed from the created `ClientRequest`; see
 * `useDiagnosticsChannel` for the resulting limitations.
 */
 ...
 * Returned attributes are added to the new client span. In
 * diagnostics-channel mode these options are reconstructed from the created
 * `ClientRequest`; see `useDiagnosticsChannel` for the resulting limitations.
 */
...
export interface HttpInstrumentationConfig extends InstrumentationConfig {
   ...
   * In this mode, outgoing hook options are reconstructed from the created
   * `ClientRequest`. They may include headers generated by Node.js and omit
   * original request options that are not exposed on `ClientRequest`.
